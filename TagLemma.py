...
class TagLemma:
    ...
    def remove_stop_words(self, input_tokens):
        self.found_stop_words = [token for token in input_tokens if token in self.STOP_WORDS]

        cleaned_tokens = [token for token in input_tokens if token not in self.STOP_WORDS]
